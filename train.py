if __name__ == "__main__":
	'''
	This script performs the training of a U-Net
	The model will be saved every 3 Epochs
	Losschart is saved under /output

	How to use:
	Specify the paths to the data you want to train and validate on when creating the datasets
    ...
    validationloader = DataLoader(
        destaster_vision_validationset,
        shuffle=False,
        batch_size=config.BATCH_SIZE,
        drop_last=True,
        pin_memory=False,
    ...
    # load model
    unet = torch.load("U-Net/U-Net2.0/unet2.0_disaster_vision_BCEWLL_.pth")
    ...
    for e in tqdm(range(config.NUM_EPOCHS)):
        ...
        print("Train loss: {:.6f}, Validation loss: {:.4f}".format(avgTrainLoss, avgValidationLoss))
