...
class DestasterVisionDataset(Dataset):
    def __init__(self, image_folder="", target_folder="", labels_folder="", transforms=transforms):
    ...
    def __len__(self):
        return len(self.images_paths_pre)


def calculate_diff_target_area_for_dataset():

    destaster_vision_dataset = DestasterVisionDataset(
        image_folder="train/images", target_folder="train/targets", labels_folder="train/labels"
    )
...
def plot_bar_chart(diff_all_images):
    ...
    for i in range(1, 5):
        ...
        print(
            f"Disaster score: {i}, median: {np.median(values):.2f}, mean: {np.mean(values):.2f}, standard deviation: {np.std(values):.2f}"
        )
